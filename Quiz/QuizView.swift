...
struct QuizView: View {
    ...
    func generateQuizItems() {
        ...
        for (index, translation) in shuffledTranslations.prefix(min(translations.count, 10)).enumerated() {
            // 日本語を問題文、英語を選択肢にする
            ...
            var availableTranslations = shuffledTranslations.filter { $0.id != translation.id }
            ...
            while wrongOptions.count < 2 && !availableTranslations.isEmpty {
                if let wrongTranslation = availableTranslations.randomElement() {
                    ...
                    if !wrongOptions.contains(wrongAnswer) && wrongAnswer != correctAnswer {
                        wrongOptions.append(wrongAnswer)
                    }
                    availableTranslations.removeAll { $0.id == wrongTranslation.id }
                }
            }
            ...
            if wrongOptions.count == 2 {
                ...
                let quizItem = QuizItem(question: question, options: options, correctAnswerIndex: correctIndex)
    ...
    var body: some View {
        ZStack {
            ...
            if quizItems.isEmpty {
                ...
                VStack {
                    ...
                    // 進捗表示
                    Text("問題 \(currentQuestionIndex + 1) / \(quizItems.count)")
                        .font(.subheadline)
                        .foregroundColor(.white.opacity(0.8))
                        .padding(.top, 5)
                    ...
                    if isShowingFeedback {
                        Text(isCorrect ? "正解！" : "不正解... 正解は「\(question.options[question.correctAnswerIndex])」")
                            .font(.headline)
                            .padding(10)
        ...
        .navigationDestination(isPresented: $showResult) {
            ResultView(score: $score, navigationPath: $navigationPath, totalQuestions: totalQuestions)
        }
